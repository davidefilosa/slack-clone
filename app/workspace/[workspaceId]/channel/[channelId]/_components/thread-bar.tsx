...
export const ThreadBar = ({
  ...
  if (!count || !timestampt) return null;
  return (
    <button
      onClick={onClick}
    ...
    >
      <div className="flex items-center gap-2 overflow-hidden">
        ...
        <span className="text-sm text-sky-700 hover:underline fold-bold truncate">
          {count} {count > 1 ? "replies" : "reply"}
        </span>
        <span className="text-xs tet-muted-foreground truncate group-hover/thread-bar:hidden block">
          Last reply{" "}
          {formatDistanceToNow(new Date(timestampt), { addSuffix: true })}
        </span>
